...
public interface DBranchInfo {
    ...
    @Query("SELECT * FROM Branch_Info WHERE sBranchCd=:BranchCde")
    ...
    @Query("SELECT * FROM Branch_Info WHERE sBranchCd LIKE 'M%' AND sTownIDxx =:Town")
    LiveData<List<EBranchInfo>> GetMCBranches(String Town);

    @Query("SELECT * FROM Branch_Info WHERE sBranchCd LIKE 'C%' AND sTownIDxx =:Town")
    LiveData<List<EBranchInfo>> GetMPBranches(String Town);

    @Query("UPDATE Branch_Info SET " +
            "sBranchNm =:BranchNm, " +
            "sDescript =:Descript, " +
            "sAddressx =:Addressx, " +
            "sContactx =:Contactx, " +
